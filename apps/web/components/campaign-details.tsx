...
export function CampaignDetails({ integration, range, onClose }: { integration: MarketingIntegration; range: DateRange; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ section, date, from: range.from || shiftDate(toBrazilDateStr(), -365), to: range.to });
  ...
  }, [endpoint, section, date, range.from, range.to, revision]);
  ...
  return <Card>
    <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">{integration.metaCampaignName}</h2><p className="text-sm text-zinc-400">{integration.appName}</p></div><button type="button" className={button} onClick={onClose}>Fechar detalhes</button></div>
    ...
    {!state.loading && data?.creatives && <>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-zinc-400">{data.range?.from} a {data.range?.to} · Instalações atribuídas pela Meta. CPI menor não comprova ROI.</p><label className="text-sm text-zinc-400">Ordenar <select value={sort} onChange={event => setSort(event.target.value)} className="rounded bg-zinc-900 p-2"><option value="spend">Maior gasto</option><option value="installs">Mais instalações</option><option value="cpi">Menor CPI</option></select></label></div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{creatives.map(ad => <article key={ad.id} className="overflow-hidden rounded-xl border border-white/10 bg-white/[0.02]">
        ...
        <div className="space-y-3 p-4"><h3 className="font-medium">{ad.name}</h3><p className="text-xs text-violet-300">{statusLabels[ad.status] || ad.status}</p>
          ...
          <a className="block text-sm text-violet-400 hover:underline" href={metaUrl(ad.id)} target="_blank" rel="noreferrer">Ver anúncio e prévia no Meta ↗</a>
        </div></article>)}</div>
    </>}

    {!state.loading && data?.events && <div className="space-y-4">
    ...
    {!state.loading && data?.windows && <div className="space-y-4">
